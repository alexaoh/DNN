...
author: "Alexander J Ohrt, Liv Breivik, Hannes Johansson"
...
# Hannes: Jag tolkar det också som att vi enbart ska fokusera på gene expression i delfrågorna 2-6. De ber ju oss att göra en ny uppdelning av patienter med både full data (gene expression och protein abundance) i deluppgift 7. Alltså tror jag på att använda set int4 som chosen.data.
```

From the code above we can see that `r length(int1)` of the patients' sample codes exist in both the protein abundance and the gene expression datasets, before removing the rows with missing values from the gene expression data. This is not interesting to us however, since we are only interested in individuals who don't have missing gene expression data. After removing the missing values from the gene expression data, the number of patients that have data available for both gene expression and protein abundance is `r length(int2)`. Moreover, `r length(int3)` of the patients' sample codes exist in all three datasets; when the `clinical` dataset is included together with the gene expression dataset and the protein abundance dataset (without considering missing values in the `clinical` data, this is handled later). This means that this amount of patients have data of both types available. But do all these individuals have data concerning their breast invasive carcinoma (BRCA) estrogen receptor status? Later, in step 7 when this set is going to be used, we remove all the individuals that don't have this information. 


If we instead disregards focus on the protein abundance for now, the number of the patients' sample codes that exist in both the the clinical dataset and in the complete gene expression dataset (i.e. when rows with missing gene expression data have been removed) are `r length(int4)`, also calculated without regards of missing values in the clinical data. This is the set that is going to be used in question 2-6 when the focus is on gene expression only. Next, we remove all the individuals that don't have data concerning their breast invasive carcinoma (BRCA) estrogen receptor status in this set.
...
The 25% percent of genes with the most variability are chosen. (Information about the genes chosen are stored and reused later in the selection of the genes for the other set in question 7, to make sure that the same of genes that the network was trained with are the ones selected for that set as well.)
...
ATT LÄGGA IN VID DEN MOTSVARANDE KODEN: As explained in section 2, the same gene selection as then is used for this set to secure that the input now consist of the same genes as the network was trained with in previous sections. 
...
```{r}
#Things gathered from the file given in class
source("aes_practice_3.R")
```
...
Har antatt at vi skal konkatinere med 2 units, vet ikke om dette stemmer
