...
for (i in seq(5, 11, 2)) {
  ...
  image(1:img_height, 1:img_height, rotate(im), col=gray((0:255)/255), axes = F, asp = 1, ylab = paste0("Label: ", batch[[2]][i]))
}
```
...
for (i in 1:4) {
  ...
  rotate <- function(x) t(apply(x, 2, rev))
  image(1:img_height, 1:img_height, rotate(im), col=gray((0:255)/255), axes = F, asp = 1, ylab = paste0("Label: ", batch[[2]][1]))
}
```
...
Notice that the encoder consists of three convolutional layers and three max pooling layers. Additionally, notice that the decoder consists of three convolutional layers, one transpose convolutional layer and two upsamplig layers. Note that the decoder uses a transpose convolutional as the first hidden layer, in order to reverse the stride of 2 used in the last max pooling layer used in the encoder. 

LEGG TIL DROPOUT ELLER LIGNENDE FOR Å UNNGÅ OVERFITTING?! (I STEDET ELLER I TILLEGG TIL DATA AUGMENTATION).
...
We compile and fit the model. Notice that when fitting this model, we have tried both with and without image augmentation on the validation data as well USIKKER PÅ OM DETTE GIR MENING HER?! MULIG OM DATA AUGMENTATION GIR EN DÅRLIG MODELL, SIDEN DET ER EN AUTOENCODER!
...
We do some predictions to give an idea of the performance of the CAE, when it comes to if it is able to recreate the input images. 
...
#plot(as.raster(output_cae[index,,,1]))
...
im <- matrix(dec_output_cifra[index,,,], nrow = 128, ncol = 128)
image(1:128, 1:128, im, col=gray((0:255)/255))
```

It is apparent that the images generated from the CAE seem to look very little like the input images, which is not a good sign. 

Next we use the CAE as pre-training model for prediction of emission or not, to see what the performance is like in this regard. 
...
model.output2 <- model.output %>% 
  layer_flatten() %>% 
  layer_dense(img_height*2,activation = "relu") %>% # Couple with fully connected layers (DNN).
  layer_dropout(0.4) %>% 
...
```{r graphicalRepr, eval = F}
# Encoder Results:
par(mfrow = c(3,3))
for(i in 1:8){
  #plot(as.raster(enc_output[2,,,i]), xlab = test.image_array_gen$classes[i])
  im <- matrix(enc_output[1,,,i], nrow = img_height, ncol = img_height)
  image(1:img_height, 1:img_height, im, col=gray((0:255)/255)) # Usikker på om denne transf skal brukes her, siden jeg allered har gjort preprocess med scaling!
}
par(mfrow = c(1,1))
```
...
```{r prøver, eval = F}
# Stuff from cifar10. 
# Extract z layer from model.CAE. 
z_layer_output <- lapply(model.CAE$layers[[1]]$layers[6], function(layer) layer$output)
activation_model <- keras_model(inputs = model_enc$input, outputs = z_layer_output)

img.data <- generator_next(test.image_array_gen)
img <- img.data[[1]][1,,,]
(label <- img.data[[2]][1])

img_tensor <- array_reshape(img,c(1,image_size)) # er dette nødvendig?
dim(img_tensor)
activations <- activation_model %>% predict(img_tensor)
z_layer_activation <- activations[[1]]
dim(z_layer_activation)
...
plot_channel(z_layer_activation[1,,,1])
# ÅPENBART NOE SOM IKKE STEMMER HER?!?!?
```
