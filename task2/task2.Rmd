...
for (i in seq(5, 11, 2)) {
  ...
  im <- matrix(batch[[1]][i,,,], nrow = 128, ncol = 128)
  rotate <- function(x) t(apply(x, 2, rev))
  image(1:128, 1:128, rotate(im), col=gray((0:255)/255), axes = F, asp = 1, ylab = paste0("Label: ", batch[[2]][i]))
}
```
...
for (i in 1:4) {
  ...
  im <- matrix(batch[[1]][1,,,], nrow = 128, ncol = 128)
  rotate <- function(x) t(apply(x, 2, rev))
  image(1:128, 1:128, rotate(im), col=gray((0:255)/255), axes = F, asp = 1, ylab = paste0("Label: ", batch[[2]][1]))
}
```
...
model_dec <- keras_model_sequential() %>% 
  ...
  layer_conv_2d_transpose(filters = 8, kernel_size = c(4,4),
      activation = "relu", padding = "valid", strides = c(2,2)) %>%
  layer_conv_2d(filters = 16, activation = "relu", 
...
callback.parameters.CAE <- callback_early_stopping(
  ...
  patience = 5, # This argument is used to interrupt training when
...
im <- matrix(output_cae[index,,,], nrow = 128, ncol = 128)
image(1:128, 1:128, im, col=gray((0:255)/255))

# Plot the real image. 
plot(as.raster(generator_next(test.image_array_gen)[[1]][index,,,]))
...
for(i in 1:8){
  ...
  im <- matrix(enc_output[1,,,i], nrow = 128, ncol = 128)
  image(1:128, 1:128, im, col=gray((0:255)/255)) # Usikker på om denne transf skal brukes her, siden jeg allered har gjort preprocess med scaling!
}
par(mfrow = c(1,1))
```
